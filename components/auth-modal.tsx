    ...
    try {
      if (isSignUp) {
        ...
        toast.success('Đăng ký thành công! 🎉', {
          description: 'Tặng ngay +50 Coins vào tài khoản của bạn.',
        });
        setIsSignUp(false);
      } else {
        // 2. ĐĂNG NHẬP FORM EMAIL
        ...
        // CHECK XEM USER CÓ BỊ BAN KHÔNG NẾU ĐĂNG NHẬP THÀNH CÔNG
        if (signInData.user) {
          ...
          if (profile?.is_banned) {
            // ĐÃ BỊ BAN -> ĐĂNG XUẤT NGAY TỨC THÌ & KHÔNG CHO VÀO
        ...
        toast.success('Đăng nhập thành công!', {
          description: 'Chào mừng bạn trở lại với SonderNet.',
        });
        onClose();
      }
    } catch (err: any) {
      ...
      <div className="bg-white w-full max-w-md max-h-[90vh] overflow-y-auto rounded-2xl p-4 sm:p-6 shadow-2xl relative space-y-4 border border-slate-200">
        ...
        <div className="space-y-1 pr-6">
          <h2 className="text-lg sm:text-xl font-extrabold text-slate-900">
            {isSignUp ? 'Tạo tài khoản SonderNet' : 'Chào mừng trở lại!'}
          </h2>
        ...
        >
          ...
          <span>{isSignUp ? 'Đăng ký nhanh bằng Google' : 'Tiếp tục với Google'}</span>
        </button>
